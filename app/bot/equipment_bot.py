from telegram import Update, ReplyKeyboardMarkup
from telegram.ext import ContextTypes, CommandHandler, MessageHandler, filters
import asyncio
from app.services.equipment_service import EquipmentService
from app.bot.equipment_card_formatter import EquipmentCardFormatter
from app.seed.mockup_equipment import MockDataGenerator
from app.models.equipment import Equipment, RentalStatus

class EquipmentBot:
    """Основной класс бота для работы с оборудованием"""
    
    def __init__(self, equipment_service: EquipmentService):
        self.equipment_service = equipment_service
        self.formatter = EquipmentCardFormatter()
        
        # Демо-данные пользователей
        self.users = {
            1: "Иван Петров",
            2: "Анна Сидорова", 
            3: "Петр Иванов",
            4: "Мария Козлова",
            5: "Сергей Смирнов",
            6: "Ольга Николаева"
        }
    
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обработчик команды /start"""
        main_keyboard = [
            ["🎒 Моё оборудование"],
            ["📦 Доступное оборудование"], 
            ["➕ Создать оборудование"]
        ]
        reply_markup = ReplyKeyboardMarkup(main_keyboard, resize_keyboard=True)
        
        await update.message.reply_text(
            "👋 Добро пожаловать в сервис аренды оборудования!\n"
            "Выберите действие:",
            reply_markup=reply_markup
        )
    
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обработчик текстовых сообщений"""
        text = update.message.text
        user_id = update.effective_user.id
        
        if text == "🎒 Моё оборудование":
            await self.show_my_equipment(update, context, user_id)
        elif text == "📦 Доступное оборудование":
            await self.show_available_equipment(update, context, user_id)
        elif text == "➕ Создать оборудование":
            await update.message.reply_text("🛠️ Функция создания оборудования в разработке...")
        else:
            await update.message.reply_text("🤔 Не понимаю команду. Используйте кнопки меню.")
    
    async def show_my_equipment(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
        """Показать оборудование текущего пользователя"""
        # В реальном приложении здесь был бы запрос к БД
        # Для демо используем моки
        mock_generator = MockDataGenerator()
        all_equipment = await mock_generator.create_mock_data(None, None)
        my_equipment = [eq for eq in all_equipment if eq.landlord_id == 1]  # user_id=1 для демо
        
        if not my_equipment:
            await update.message.reply_text("📭 У вас пока нет оборудования.")
            return
        
        await update.message.reply_text(f"🎒 Ваше оборудование ({len(my_equipment)} позиций):")
        
        for equipment in my_equipment[:3]:  # Показываем первые 3 для демо
            card_text = self.formatter.create_equipment_card(
                equipment, 
                self.users.get(equipment.landlord_id, "Вы")
            )
            
            # Для своего оборудования не показываем кнопки бронирования
            await update.message.reply_text(
                card_text,
                parse_mode='Markdown'
            )
    
    async def show_available_equipment(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
        """Показать доступное оборудование других пользователей"""
        # В реальном приложении здесь был бы запрос к БД
        # Для демо используем моки
        mock_generator = MockDataGenerator()
        all_equipment = await mock_generator.create_mock_data(None, None)
        available_equipment = [
            eq for eq in all_equipment 
            if eq.landlord_id != 1 and eq.status == RentalStatus.AVAILABLE  # user_id=1 для демо
        ]
        
        if not available_equipment:
            await update.message.reply_text("😔 Сейчас нет доступного оборудования.")
            return
        
        await update.message.reply_text(f"📦 Доступное оборудование ({len(available_equipment)} позиций):")
        
        for equipment in available_equipment[:3]:  # Показываем первые 3 для демо
            card_text = self.formatter.create_equipment_card(
                equipment, 
                self.users.get(equipment.landlord_id, "Неизвестно")
            )
            
            keyboard = self.formatter.create_interaction_keyboard(equipment.id)
            
            await update.message.reply_text(
                card_text,
                parse_mode='Markdown',
                reply_markup=keyboard
            )
    
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обработчик callback-запросов от inline кнопок"""
        query = update.callback_query
        await query.answer()
        
        callback_data = query.data
        
        if callback_data.startswith('book_'):
            equipment_id = int(callback_data.split('_')[1])
            await query.edit_message_text(
                f"📋 Бронирование оборудования ID: {equipment_id}\n"
                f"Функция бронирования в разработке..."
            )
        elif callback_data.startswith('ask_'):
            equipment_id = int(callback_data.split('_')[1])
            await query.edit_message_text(
                f"💬 Вопрос по оборудованию ID: {equipment_id}\n"
                f"Функция вопросов в разработке..."
            )