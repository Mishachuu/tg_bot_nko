...
class UserRepository:
    ...
    async def update(self, session: AsyncSession, user_id: int, changes: dict) -> AppUser | None:
        # ИСПРАВЛЕННЫЙ КОД: используем update вместо sql_update
        stmt = (
            update(self.model)
    ...
    async def delete_by_id(self, session: AsyncSession, user_id: int) -> bool:
        # ИСПРАВЛЕННАЯ ОШИБКА: было self.table, должно быть self.model
        res = await session.execute(select(AppUser).where(self.model.id == user_id))
    ...
    async def get_users_paginated(self, session: AsyncSession, skip: int = 0, limit: int = 100, is_lessor: Optional[bool] = None) -> List[AppUser]:
        """Получить пользователей с пагинацией и фильтрацией"""
        stmt = select(self.model)
        
        # Добавляем фильтр по статусу арендодателя если передан
        if is_lessor is not None:
            stmt = stmt.where(self.model.is_lessor == is_lessor)
